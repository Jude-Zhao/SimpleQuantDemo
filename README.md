...
| 模块 | 说明 |
|------|------|
| `core/` | 核心计算层：因子库（插件式注册）、分析（IC/ICIR）、合成、优化器（等权/MVO/BL）、约束校验、回测引擎 |
...
2. **三种策略运行** — 线性因子策略、MVO 均值方差优化、Black-Litterman
3. **标的池管理** — 添加/移除 ETF、分类标签
4. **插件式因子库** — 因子注册表 + 自动发现
5. **分类约束体系** — 分类规则引擎、约束校验、优化器约束集成
